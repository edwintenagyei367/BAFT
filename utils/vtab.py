...
def get_vtab_data(name, evaluate=False, normalize=True, resize=224, batch_size=64, num_workers=8, is_hdf5=True):
    if name in _vtab_class_num_dict:
        root = os.path.join('/media/edwin-s-lab/0A1CF60D1CF5F393/Datasets/','vtab_h5', name)
        if normalize:
