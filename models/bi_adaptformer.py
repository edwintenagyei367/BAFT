...
def set_bi_adapter(model, dim=32, s=1, bit=1, feature="token_mean"):
    for layer in model.children():
        if type(layer) == timm.models.vision_transformer.Block:
            hidden_dim = layer.norm2.normalized_shape[0]
            layer.adapter_mlp = Adapter(in_dim=hidden_dim, dim=dim, bit=bit)
            layer.s = s
            ...
            layer.adapter_mlp = Adapter(in_dim=layer.dim, dim=dim, bit=bit)
            layer.s = s
            layer.graph_layer = GraphLayer(feature_to_use=feature)
            bound_method = forward_swin_block.__get__(layer, layer.__class__)
            setattr(layer, 'forward', bound_method)
        elif len(list(layer.children())) != 0:
            set_bi_adapter(layer, dim, s, bit, feature)
